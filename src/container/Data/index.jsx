...
const Data = () => {
	...
	const getData = async () => {
		...
		// 过滤支出和收入
		const expense_data = data.total_data.filter(item => item.type == 1).sort((a, b) => b.number - a.number); // 过滤出账单类型为支出的项
		const income_data = data.total_data.filter(item => item.type == 2).sort((a, b) => b.number - a.number); // 过滤出账单类型为收入的项
	...
	const selectMonth = (item) => {
  	...
  	return <div className={s.data}>
		...
		<div className={s.structure}>
      		...
      		{
      			(totalType == 'expense' ? expenseData : incomeData).map(item => <div key={item.label_id} className={s.item}>
